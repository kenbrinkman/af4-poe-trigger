...
| — | **12 V panel-mount jack**, 5.5 × 2.5 mm centre-positive, threaded panel barrel | panel, off-board | **New 2026-09-18, not yet sourced — item 28.** Replaces J1 as the input. Thread must clear a 3.0 mm wall. Measure it and set the `PJ_*` block in `af4_enclosure_ocp.py` before printing the case |
| — | **Polarised 2-pin inline connector** (JST-XH or equivalent) + hook-up wire | inline | **New 2026-09-18 — item 28.** Lets the hat lift off its headers without unsoldering. Polarised so the pair cannot go back reversed |
