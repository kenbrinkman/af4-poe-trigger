...
| J1 | **Same Sky PJ-079BH**, 5.5 × 2.5 mm, 24 V / 5 A | THT right-angle | ⚠️ **Fitted 180° backwards on all five rev E boards and left in place, dead** (§A4). Its pads are still the 12 V entry point, tapped from the hat's underside. **Watch the trap:** PJ-002AH and PJ-102AH are the 2.0–2.1 mm parts. Only the "B" suffix is 2.5 mm centre pin. Same trap as the splitter |
| — | **DALQUIS DC-099**, 5.5 × 2.5 mm, 10 A, threaded panel mount, 150 mm 18 AWG pre-soldered leads | panel, off-board | **Replaces J1 as the 12 V input, 2026-09-18 — already in stock, six of them, and rev C used the same part.** **Red is +12 V, black is −.** Barrel Ø11.50 with a **flat** at 10.50 (the case hole is a D, flat up); nut 3.00 thick, 14.00 across flats. → §A4 |
| — | *Optional:* polarised 2-pin inline connector rated for 18 AWG (JST-VH, **not** XH) | inline | Only if the 150 mm leads do not give enough slack to lift the hat off its headers. Check at the dry fit before buying |
