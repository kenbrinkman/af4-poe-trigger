...
- `hardware/enclosure/aF4-trigger-case.stl` (+ `.step`) + `hardware/enclosure/aF4-trigger-lid.stl` (+ `.step`) — printed enclosure, **rev D** (PETG, 65.2 x 117.0 x 39.9 mm): flush RJ45 on the input wall; the trigger hat's barrel jack and 3.5 mm jack exit one long wall; two tall standoffs carry the hat; four light-pipe LED sight holes in the lid (D3, D5 on the hat; PWR1, LNK1 on the Olimex board). The rev C protoboard bay, buck pocket, DC-099 hole and PG7 gland are all gone. Details in `aF4-enclosure-notes.md`; parametric source `hardware/enclosure/af4_enclosure_ocp.py`, which asserts its own fit checks before exporting.
