...
| 12V DC splitter | Barrel Y-splitter, **5.5×2.5mm, center-positive (confirmed)** | $6 | Taps feeder's 12V. Measured OEM trigger is **10.37V, not 12V** — regulate the tap down (next row). Note: many cheap Y-splitters are 5.5×2.1mm — 2.1 seats loosely in 2.5 jacks; buy 2.5mm specifically. |
...
| TVS diode | **SMAJ12A** (unidirectional) | $0.50 | Spliced across the trigger pair **~1" behind the plug**, under heatshrink — the slim MP3-3501 barrel is too cramped for internal mounting. Cathode (striped end)→tip conductor, anode→sleeve. Clamps ESD/inductive spikes after the cable run. Through-hole alternative: P6KE12A. |
...
| Enclosure | **3D-printed case** (`aF4-trigger-case.stl` + `aF4-trigger-lid.stl`, PETG) + 2× PG7 cable glands + 4× M3×12 self-tapping screws | ~$3 glands | 51.7 × 155 × 38.9 mm, flush RJ45, dry-location (no gasket). See `aF4-enclosure-notes.md` for print/assembly details. |
