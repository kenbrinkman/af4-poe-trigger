...
~95 KB and reading it costs ~24,000 tokens. Use its section index, then `grep -n` the
...
- **Finishing work:** **rewrite `STATUS.md`, do not append to it.** The durable record goes to
  ...
  section index is regenerated by `tools/section_index.py`, which a Stop hook runs every turn;
  a pre-commit hook blocks `git commit` while it is stale (`docs/git-rules.md` §6).
- **Never renumber `§N.N`.** Docs, commits and conversations cite them. Add; do not renumber.
- **Tools run here, so verify rather than trust the record** (§9.2): enclosure scripts in
  `~/.venvs/cad`, `pcbnew` via KiCad 9's bundled Python, `esphome config` locally (stdout to
  `/dev/null` — it may print secrets), and live Home Assistant state through its MCP server.
  **Read-only against Home Assistant unless Kenny asks** — `input_boolean.reef_af4_schedule_enabled`
  is the go-live switch, and a scheduled-feed automation drives real hardware.
...
| Check HA automations / helpers against reality | §5.7, then the Home Assistant MCP tools — read-only |
...
| Toolchain: what is installed, where, what runs (`pcbnew`, `kicad-cli`, OCP, ESPHome) | §9.2, then §9.1 for history |
