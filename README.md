...
ESP32-POE-ISO → GPIO32 → 220Ω → AQY212GS PhotoMOS → switches the feeder's own 12V supply (regulated to ~10.4V by an on-board LM1117-ADJ) onto the 3.5mm trigger jack. Safety guardrails (10s pulse, 5-min lockout, restore-off) are baked into the ESPHome config on-device; HA is scheduler only.

**Rev D** puts all of that on a PCB "hat" that plugs onto the ESP32's EXT1/EXT2 headers and carries both panel connectors, assembled by PCBWay. The rev C hand-wired protoboard is retired.

> The trigger pin is **GPIO32**, not GPIO13. The Olimex board has a factory 2.2 kΩ pull-up on GPIO13 that can partially turn the PhotoMOS on during boot. See `aF4-rev-D-pcb-notes.md`.
...
| `aF4-trigger-case.stl/.step`, `aF4-trigger-lid.stl/.step` | Printable enclosure (rev D) |
...
`pcb/af4-trigger-hat-rev-D-PCBWay.zip` is what you upload: Gerbers, drill, BOM,
centroid, fab notes.
